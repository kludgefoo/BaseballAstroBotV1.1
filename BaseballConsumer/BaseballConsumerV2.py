...
class BaseballUpdaterBotV2:
    ...
    def funEmoji(self, info):
        print(info)
        emoji = ""
        # lookup batter team id
        batterTeamID = self.lookupPlayerTeam(info['batterID'])
        ...
        if info['event'] == 'Home Run' and info['rbi'] == '4': # "grand slam" in info['description']:
            # determine which team the HR belongs to, if not good guys, don't emoji/use different emoji
            if batterTeamID == self.TEAM_ID:
